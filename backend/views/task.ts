...
export default function (io: Server) {
    ...
    router.post("/new", async (_req, _res) => {
        ...
        if (name == "" || typeof name !== "string" || (typeof description !== "string" && description !== undefined))
    ...
    router.post("/move", async (_req, _res) => {
        const id = _req.body.id;
        const category_id = _req.body.category_id;
        if (id == "" || category_id == "" || typeof category_id !== "number" || typeof id !== "number")
            return _res.status(400).json({ error: "no category id or id given or bad types" });
        try {
            const result = await prisma.task.update({
                where: {
                    id: id,
